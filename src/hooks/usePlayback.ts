...
import type { PlaybackSnapshot, PlaybackStatus } from "../playback/types";
...
export function usePlayback(activeResult: SearchResult | null): PlaybackController {
  ...
  const timeline = useMemo(() => {
    return activeResult && activeResult.events.length > 0
      ? createPlaybackTimeline(activeResult.events)
      : null;
  }, [activeResult]);
  ...
  const play = useCallback(() => {
    ...
    // Do not restart if already at the completed final state
  ...
  const previousStep = useCallback(() => {
    ...
    // If cursor was in the middle of a step (e.g. paused mid-animation),
    // jump back to the boundary of this current step.
    // Otherwise, step back to the previous step boundary.
    const targetStep = cursor > baseCursor
      ? currentStep
  ...
  useEffect(() => {
    if (!isPlaying || !activeResult || !timeline || cursor >= activeResult.events.length) {
      clearActiveTimer();
      return;
    }
  ...
  const isComplete = Boolean(activeResult && cursor >= activeResult.events.length && activeResult.events.length > 0);
